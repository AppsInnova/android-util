...
import com.yy.androidlib.util.http.HttpMultipartEntity;
 ...
 * {@link com.loopj.android.http.AsyncHttpClient} instance. <p>&nbsp;</p> For example: <p>&nbsp;</p>
 * <pre>
...
public class RequestParams implements Serializable {
    ...
    protected final ConcurrentHashMap<String, Object> fileParams = new ConcurrentHashMap<String, Object>();
    ...
    public void put(String key, File file, String contentType, String customFileName) throws FileNotFoundException {
        ...
        if (key != null) {
            FileWrapper wrapper = new FileWrapper(file, contentType, customFileName);
            Object value = fileParams.get(key);
            if (value != null) {
                if (value instanceof FileWrapper) {
                    List<FileWrapper> list = new ArrayList<FileWrapper>();
                    list.add((FileWrapper) value);
                    list.add(wrapper);
                    fileParams.put(key, list);
                } else {
                    List<FileWrapper> list = (List<FileWrapper>) value;
                    list.add(wrapper);
                }
            } else {
                fileParams.put(key, wrapper);
            }
        }
    }
    ...
    public String toString() {
        ...
        for (ConcurrentHashMap.Entry<String, Object> entry : fileParams.entrySet()) {
            if (result.length() > 0)
                result.append("&");
    ...
    private HttpEntity createMultipartEntity(ResponseHandlerInterface progressHandler) throws IOException {
        HttpMultipartEntity entity = new HttpMultipartEntity(Charset.forName("UTF-8"));
        ...
        List<BasicNameValuePair> params = getParamsList(null, urlParamsWithObjects);
        for (BasicNameValuePair kv : params) {
            entity.addStringPart(kv.getName(), kv.getValue());
        }
...
//                        stream.contentType);
        ...
        for (ConcurrentHashMap.Entry<String, Object> entry : fileParams.entrySet()) {
            if (entry.getValue() instanceof List) {
                for (FileWrapper value : (List<FileWrapper>) entry.getValue()) {
                    entity.addFileParts(entry.getKey(), value.file.getAbsolutePath());
                }
            } else {
                FileWrapper fileWrapper = (FileWrapper) entry.getValue();
                entity.addFileParts(entry.getKey(), fileWrapper.file.getAbsolutePath());
            }
